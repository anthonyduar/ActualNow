...
export default async function CookiesPage() {
  ...
  // 1. Bloque seguro para traer la página de Cookies
  try {
    const res = await fetch(
      `${baseUrl}/pages?slug=politica-de-cookies-an&_embed`,
      {
        cache: "no-store",
      },
    );
    ...
    if (res.ok) {
      ...
      page = Array.isArray(data) ? data[0] : null;
    }
  } catch (error) {
    console.warn("Petición de la página de cookies interrumpida o fallida.");
  }
  ...
  try {
    ...
    console.warn("Petición de recomendados en cookies interrumpida o fallida.");
  }
