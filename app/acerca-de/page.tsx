...
export default async function AcercaDePage() {
  ...
  // 1. Bloque seguro para traer el contenido de Acerca De
  try {
    const res = await fetch(`${baseUrl}/pages?slug=acerca-de&_embed`, {
      cache: "no-store",
    });
    ...
    console.warn("Petición del contenido de 'Acerca de' interrumpida o fallida.");
  }
  ...
  try {
    ...
    console.warn("Petición de recomendados en 'Acerca de' interrumpida o fallida.");
  }

  // Si no se encuentra la página por caída de red, muestra un aviso elegante
  if (!page)
    return (
      <div className='p-20 text-center text-white font-sans min-h-screen bg-zinc-950 flex flex-col items-center justify-center gap-4'>
        <p>Página Acerca de no disponible temporalmente.</p>
